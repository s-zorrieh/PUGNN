...
class Analyzer(BaseAnalyzer):
    ...
    def LLEVisual(self, av_pu, f=0.6):
        ...
        fig.update_layout(title=f"$\lambda=<PU>=L\sigma={av_pu}$")

        s = namedtuple('LLEVisual',
                       ['y', 'yhat', 'generator_dist', 'predicted_dist', 'plot'])
        
        directory = osp.join(self._output_dir, 'LLEVisual.html')
        i = 0
        while directory in os.listdir(path=self._output_dir):
            directory = osp.join(self._output_dir, f'LLEVisual{i}.html')
        plotly.io.write_html(fig, directory)
        plotly.io.write_json(fig, directory)
